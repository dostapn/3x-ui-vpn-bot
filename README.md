...
- `/start` с главным меню и подсказкой об обратной связи
- Просмотр ключей с VLESS конфигами (Reality параметры)
- Запрос новых ключей (без дублирования запросов)
- Статистика использования трафика
- Автоуведомления при одобрении

**Административная функциональность:**
- Выдача новых ключей с выбором/клонированием inbound
- Присвоение существующих ключей
- Команда `/requests` - список незавершённых запросов
- Команда `/keys` - список всех ключей с пользователями
- Команда `/bans` - список заблокированных
- Управление пользователями ключей (отвязка, бан)
- Блокировка пользователей (24ч)
- Отклонение запросов (с блокировкой и без)
- Отправка сообщений пользователям

**База данных:**
- Таблицы: `telegram_users`, `user_keys`, `pending_requests`
- Связь "один ключ → много пользователей"
- Интеграция с существующей БД 3x-ui

**API интеграция:**
- Полная интеграция с 3x-ui REST API (py3xui)
- Генерация VLESS URL с Reality параметрами
- Управление inbound'ами и клиентами
- Получение статистики трафика

**Развертывание:**
- Docker Compose для разработки
- systemd service для продакшн
- Автоинициализация БД
- Централизованное логирование

**Технические детали:**
- Python 3.10+, aiogram 3.13.1, py3xui 0.5.5
